...
import { SQL_DIALECTS, normalizeDialect } from "./sql.ts";

/** One custom migration slot for a dialect: a 1-based `order`, a `name`, and the paired `up`/`down` SQL. */
...
function customMigrationsDirFor(
...
async function discoverCustomMigrations(
  deterministicDir: string,
  dialect: string,
...
/** Reads each discovered pair's up/down SQL text. */
